...
class CycleGANModel(BaseModel):
    ...
    def forward(self):
        ...
        self.seg_A = self.netS_A(self.fake_B)       # S{G_A(A), Y_A}
        self.seg_fake_A = self.netS_A(self.rec_A)   # S{G_B(G_A(A)), Y_A}
        self.seg_B = self.netS_B(self.fake_A)       # S{G_B(B), Y_B}
        self.seg_fake_B = self.netS_B(self.rec_B)   # S{G_A(G_B(B)), Y_A}
    ...
    def backward_S(self):
        ...
        self.loss_S_A = loss(self.seg_A, torch.squeeze(self.real_seg_A, 1).long())
        self.loss_S_B = loss(self.seg_B, torch.squeeze(self.real_seg_B, 1).long())    
        self.loss_S_AB = loss(self.seg_fake_A, torch.squeeze(self.real_seg_A, 1).long())
        self.loss_S_BA = loss(self.seg_fake_B, torch.squeeze(self.real_seg_B, 1).long())
        # combined loss and calculate gradients
    ...
    def optimize_parameters(self):
        ...
        # S_A and S_B
        self.set_requires_grad([self.netS_A, self.netS_B], False) 
